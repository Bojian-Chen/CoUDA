...
def incremental_train_spilt(args, model, model_old, src_trainloader, tgt_trainloader, src_testloader , testloader, tg_optimizer, tg_lr_scheduler, weight_per_class=None, device = None):
    T = 2
    beta = 0.25
    the_lambda = 5
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx,((source_data, source_label),(target_data, target_label)) in enumerate(zip(src_trainloader,tgt_trainloader)): 
            ...
            old_outputs, old_features = model(source_data)
            new_outputs, new_features = model(target_data)
            ...
            # src_outputs_old, old_features = model_old(source_data)
            
            # classification loss
            loss1 = nn.CrossEntropyLoss(weight_per_class)(old_outputs, source_label) 
            # contrastive_loss
            loss2 = contrastive_loss(new_features[batch_size : ],new_features[batch_size : ])+contrastive_loss(new_features[ : batch_size],new_features[ : batch_size])-2*contrastive_loss(new_features[ : batch_size],new_features[batch_size : ])
            # loss of new model
