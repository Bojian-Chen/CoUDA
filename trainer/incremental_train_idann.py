...
def incremental_train_idann(args, model, src_trainloader, tgt_trainloader, src_testloader , testloader, tg_optimizer, tg_lr_scheduler, weight_per_class=None, device = None):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx,((source_data, source_label),(target_data, target_label)) in enumerate(zip(src_trainloader,tgt_trainloader)): 
            ...
            # src_features = new_features[ : batch_size]
            # tgt_features = new_features[batch_size : ]
            # _, src_d_predicted = domain_output[ : batch_size].max(1)
            # _, tgt_d_predicted = domain_output[batch_size : ].max(1)


            #Compute classification loss
            loss1 = nn.CrossEntropyLoss(weight_per_class)(src_outputs, source_label) + CORAL(src_features,tgt_features)
