...
def incremental_train_everadapt(args, model, model_old, src_trainloader, tgt_trainloader, src_testloader , testloader, exemplar_dataset, tg_optimizer, tg_lr_scheduler, weight_per_class=None, device = None):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx,((source_data, source_label),(target_data, target_label)) in enumerate(zip(src_trainloader,tgt_trainloader)): 
           ...
           # classification loss
            ...
            loss2 = ConditionalEntropyLoss(tgt_features)
            # LMMD loss
            loss3 = lmmd(src_features, tgt_features, source_label, F.softmax(new_outputs, dim=1), args.nb_cl) 
            # Distillation Loss
            loss4  = nn.CrossEntropyLoss(weight_per_class)(examplar_outputs, examplar_label) * 0.1
