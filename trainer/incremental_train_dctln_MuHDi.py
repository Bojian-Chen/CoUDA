...
def incremental_train_MuHDi(args, model, model_old, src_trainloader, tgt_trainloader, src_testloader , testloader, tg_optimizer, tg_lr_scheduler, weight_per_class=None, device = None):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx,((source_data, source_label),(target_data, target_label)) in enumerate(zip(src_trainloader,tgt_trainloader)): 
            ...
            T = 2
            old_src_outputs,_ = model_old(source_data)
            loss4 = nn.KLDivLoss()(F.log_softmax(src_outputs/T, dim=1), F.softmax(old_src_outputs/T, dim=1)) * T * T
