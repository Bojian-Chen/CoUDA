...
def incremental_train_mmda(args, model, model_old, src_trainloader, tgt_trainloader, src_testloader , testloader, tg_optimizer, tg_lr_scheduler, weight_per_class=None, device = None):
    T = 2
    beta = 0.25
    the_lambda = 5
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx,((source_data, source_label),(target_data, target_label)) in enumerate(zip(src_trainloader,tgt_trainloader)): 
            ...
            # mmd loss
            # loss2 = lmmd(src_features,tgt_features)
            loss2 = lmmd(src_features, tgt_features, source_label, F.softmax(new_outputs, dim=1), args.nb_cl) 

            loss3 = ConditionalEntropyLoss(tgt_features)
