...
def incremental_train_cua_mmd(args, model, model_old, src_trainloader, tgt_trainloader, src_testloader , testloader, exemplar_dataset, tg_optimizer, tg_lr_scheduler, weight_per_class=None, device = None):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx,((source_data, source_label),(target_data, target_label)) in enumerate(zip(src_trainloader,tgt_trainloader)): 
            ...
            # CORAL loss
            loss2 = ammd(src_features,tgt_features)
            # replay loss

            loss3 = nn.CrossEntropyLoss(weight_per_class)(examplar_outputs, examplar_label) * 0.01
            ...
            train_loss_distillation += loss3.item()
            tg_optimizer.step()
        ...
        print('Train set: {} train loss: {:.4f}  cls {:.4f}   coral loss {:.4f}  distillation {:.4f} src_accuracy: {:.4f} tgt_accuracy: {:.4f} '.format(
            len(src_trainloader), 
            train_loss/(batch_idx+1), 
            train_loss_cls/(batch_idx+1), 
            train_loss_coral/(batch_idx+1),
