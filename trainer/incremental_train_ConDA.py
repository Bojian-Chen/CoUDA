...
def incremental_train_ConDA(args, model, model_old, src_trainloader, tgt_trainloader, src_testloader , testloader, exemplar_dataset, tg_optimizer, tg_lr_scheduler, weight_per_class=None, device = None):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx,((source_data, source_label),(target_data, target_label)) in enumerate(zip(src_trainloader,tgt_trainloader)): 
            ...
            loss = loss1 + loss2 * 0.5 - loss3 *0

            _, predicted = tgt_outputs.max(1)
        ...
        print('Train set: {} train loss: {:.4f} train loss cls {:.4f} train loss Entropy {:.4f} train loss DivEntropy {:.4f} accuracy: {:.4f} '.format(
            len(tgt_trainloader), train_loss/(batch_idx+1), train_loss_cls/(batch_idx+1), train_loss_entropy/(batch_idx+1), train_loss_diventropy/(batch_idx+1), 100.*correct/total))
